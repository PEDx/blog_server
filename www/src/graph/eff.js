...
EffectPass.prototype.MakeHeader_Image = function() {
  ...
  this.mImagePassFooter +=
    ...
    'color.w = 1.0;'
  if (this.mIs20) this.mImagePassFooter += 'outColor = color;}'
  else this.mImagePassFooter += 'gl_FragColor = color;}'
  ...
  this.mImagePassFooterVR +=
    ...
    'color.w = 1.0;'
  if (this.mIs20) this.mImagePassFooterVR += 'outColor = color;}'
  else this.mImagePassFooterVR += 'gl_FragColor = color;}'
...
EffectPass.prototype.NewShader_Image = function(shaderCode, commonShaderCodes) {
  ...
  if (n1 > 0 || n2 > 0 || n3 > 0) {
    ...
    if (this.mIs20)
      ...
      vsSourceVR =
        'attribute in vec2 pos; void main() { gl_Position = vec4(pos.xy,0.0,1.0); }'
...
EffectPass.prototype.NewShader = function(shaderCode, commonSourceCodes) {
  ...
  if (this.mType == 'sound')
    res = this.NewShader_Sound(shaderCode, commonSourceCodes)
...
EffectPass.prototype.NewTexture = function(
  ...
  if (url == null || url.mType == null) {
    ...
    var returnValue = {
      mFailed: false,
      mNeedsShaderCompile:
        this.mInputs[slot] == null || this.mInputs[slot].mInfo.mType != 'volume'
    }
    this.DestroyInput(slot)
    this.mInputs[slot] = texture
    this.MakeHeader()
    ...
    texture.audio.addEventListener('error', function(e) {
      ...
      if (texture.audio.hasFalled == true) {
        /*alert("Error: cannot load music" ); */ return
      }
...
EffectPass.prototype.Paint_Sound = function(wa, d) {
  ...
  var l2 = this.mRenderer.SetShaderConstantLocation(
    this.mProgram,
    'iBlockOffset'
  )
  this.mRenderer.SetShaderConstant4FV('iDate', dates)
  ...
  for (var j = 0; j < numBlocks; j++) {
    ...
    for (var i = 0; i < numSamples; i++) {
      bufL[off + i] =
        -1.0 +
        (2.0 * (this.mData[4 * i + 0] + 256.0 * this.mData[4 * i + 1])) /
          65535.0
      bufR[off + i] =
        -1.0 +
        (2.0 * (this.mData[4 * i + 2] + 256.0 * this.mData[4 * i + 3])) /
          65535.0
    }
  }
...
EffectPass.prototype.SetUniforms = function(
  ...
  for (var i = 0; i < this.mInputs.length; i++) {
    ...
    if (inp == null) {
      ...
      if (inp.loaded == true) texID[i] = inp.globject
    } else if (inp.mInfo.mType == 'webcam') {
      if (inp.loaded == true) {
        if (inp.mImage !== null) {
          texID[i] = inp.globject
...
function Effect(
  ...
  var kayboardTexture = this.mRenderer.CreateTexture(
    this.mRenderer.TEXTYPE.T2D,
    256,
    3,
...
Effect.prototype.ResizeCubemapBuffer = function(i, xres, yres) {
  ...
  if (
    ...
    var texture1 = this.mRenderer.CreateTexture(
    ...
    var target1 = this.mRenderer.CreateRenderTargetCubeMap(
      texture1,
      null,
      false
    )

    var texture2 = this.mRenderer.CreateTexture(
      this.mRenderer.TEXTYPE.CUBEMAP,
      xres,
      yres,
      this.mRenderer.TEXFMT.C4F16,
    ...
    var target2 = this.mRenderer.CreateRenderTargetCubeMap(
      texture2,
      null,
      false
    )
  ...
  for (var j = 0; j < numPasses; j++) {
    ...
    if (rpass.type == 'buffer')
      rpassName =
        'Buffer ' +
        String.fromCharCode(
          65 + assetID_to_bufferID(this.mPasses[j].mOutputs[0])
        )
